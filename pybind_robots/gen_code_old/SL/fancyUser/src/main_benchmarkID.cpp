...
int main(int argc, char**argv)
{
    ...
    cout << "SL:" << endl
    << desiredState[jA].uff << endl
    << desiredState[jB].uff << endl
    << desiredState[jC].uff << endl
    << desiredState[jD].uff << endl
    << desiredState[jE].uff << endl
        ;
   return 1;
   //*/
...
static void matlabLog(int numOfTests, int* iterations, double* tests, const std::string& subject) {
    std::string fileName = "Fancy_" + subject + "_speed_test_data.m";
    ofstream out(fileName.c_str());
    out << "fancy_test.robot       = 'Fancy';" << std::endl;
    out << "fancy_test.description = 'test of the speed of the calculation of: " << subject << "';" << std::endl;
    out << "fancy_test.software    = 'SL';" << std::endl;
    ...
    out << "fancy_test.date = '" << timeStr << "';" << std::endl;

    out << "fancy_test.iterations = [";
    for(int t=0; t<numOfTests; t++) {
        out << " " << iterations[t];
    }
    out << "];" << endl;
    out << "fancy_test.times = [";
    for(int t=0; t<numOfTests; t++) {
        out << " " << tests[t];
    }
    out << "];" << endl;
