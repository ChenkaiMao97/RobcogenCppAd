...
//         << currentState[::jC].thdd << endl
//         << currentState[::jD].thdd << endl
//         << currentState[::jE].thdd << endl;
//      cout << endl << qdd << endl;
//      return 1;
...
void fillState(JointState& q, JointState& qd, JointState& tau, SL_Jstate* state) {
    ...
    q(Fancy::JA)   = ( ((double)std::rand()) / RAND_MAX) * max;
    qd(Fancy::JA)  = ( ((double)std::rand()) / RAND_MAX) * max;
    tau(Fancy::JA) = ( ((double)std::rand()) / RAND_MAX) * max;

    state[::jA].th  =   q(Fancy::JA);
    state[::jA].thd =  qd(Fancy::JA);
    state[::jA].u   = tau(Fancy::JA);

    q(Fancy::JB)   = ( ((double)std::rand()) / RAND_MAX) * max;
    qd(Fancy::JB)  = ( ((double)std::rand()) / RAND_MAX) * max;
    tau(Fancy::JB) = ( ((double)std::rand()) / RAND_MAX) * max;

    state[::jB].th  =   q(Fancy::JB);
    state[::jB].thd =  qd(Fancy::JB);
    state[::jB].u   = tau(Fancy::JB);

    q(Fancy::JC)   = ( ((double)std::rand()) / RAND_MAX) * max;
    qd(Fancy::JC)  = ( ((double)std::rand()) / RAND_MAX) * max;
    tau(Fancy::JC) = ( ((double)std::rand()) / RAND_MAX) * max;

    state[::jC].th  =   q(Fancy::JC);
    state[::jC].thd =  qd(Fancy::JC);
    state[::jC].u   = tau(Fancy::JC);

    q(Fancy::JD)   = ( ((double)std::rand()) / RAND_MAX) * max;
    qd(Fancy::JD)  = ( ((double)std::rand()) / RAND_MAX) * max;
    tau(Fancy::JD) = ( ((double)std::rand()) / RAND_MAX) * max;

    state[::jD].th  =   q(Fancy::JD);
    state[::jD].thd =  qd(Fancy::JD);
    state[::jD].u   = tau(Fancy::JD);

    q(Fancy::JE)   = ( ((double)std::rand()) / RAND_MAX) * max;
    qd(Fancy::JE)  = ( ((double)std::rand()) / RAND_MAX) * max;
    tau(Fancy::JE) = ( ((double)std::rand()) / RAND_MAX) * max;

    state[::jE].th  =   q(Fancy::JE);
    state[::jE].thd =  qd(Fancy::JE);
    state[::jE].u   = tau(Fancy::JE);
