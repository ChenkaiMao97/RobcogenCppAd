...
static void matlabLog(int numOfTests, int* iterations, double* tests, const std::string& subject) {
    std::string fileName = "Acrobot_" + subject + "_speed_test_data.m";
    ofstream out(fileName.c_str());
    out << "acrobot_test.robot       = 'Acrobot';" << std::endl;
    out << "acrobot_test.description = 'test of the speed of the calculation of: " << subject << "';" << std::endl;
    out << "acrobot_test.software    = 'SL';" << std::endl;
    ...
    out << "acrobot_test.date = '" << timeStr << "';" << std::endl;

    out << "acrobot_test.iterations = [";
    for(int t=0; t<numOfTests; t++) {
        out << " " << iterations[t];
    }
    out << "];" << endl;
    out << "acrobot_test.times = [";
    for(int t=0; t<numOfTests; t++) {
        out << " " << tests[t];
    }
    out << "];" << endl;
